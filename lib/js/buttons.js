...
GridBoard.prototype.candidateRemove = function(val, cands, type){
    for(var i = 0; i< this.grids.length; i++){
        if(this.grids[i].isCandidateMatch(cands, type)){
            //console.log("found candidate match for ", cands);
            this.grids[i].removeExcept(val, cands);
        }
    }
};
...
Grid.prototype.removeExcept = function(theVal, cands){
    //for every square in the grid
    for(var i = 0; i < this.gridSquares.length; i++){
        //if the square is not a candidate
        if(cands.indexOf(this.gridSquares[i]) < 0){
            //remove the possibility
            //console.log("removing ", theVal, " from ", this.gridSquares[i]);
            this.board[this.gridSquares[i]].removePoss(theVal);
        }
    }
}

Grid.prototype.isCandidateMatch = function(theArr, type){
    //needs to be other type (line/square) than input
     if(this.type == type){
         return false
     }
    // needs to contain all items in the candidate array
    for(var i = 0; i < theArr.length; i++){
        if(this.gridSquares.indexOf(theArr[i]) < 0){
            return false
        }
    }
...
Grid.prototype.reconcile = function(){
    ...
    //see if you can do a candidate line solve
    this.possMatrix.candidateLine();
...
function PossMatrix(gridInp){
    ...
    this.members = gridInp.gridSquares;
    this.board = gridInp.board;
    this.gridType = gridInp.type;
    this.puzzle = gridInp.puzzle;
    this.matrix = [];
    this.update();
}
...
function isSameMiniRow(inp){
    //are they in the same mini 3 sq row?


    var theMini = Math.floor(inp[0]/3);
    //console.log("theMini: ",theMini);
    //for the 2nd (and possible 3rd) in array

    for (var i = 1; i<inp.length; i++){
        //is this next item in the same group of 3?
        if(Math.floor(inp[i]/3) != theMini){
            //console.log("falied");
            return false
        }
    }
   // console.log("passed");
    //we made it out of the tests so we're true
    return true;
}
function isSameMiniCol(inp){
    //are they in the same mini 3 sq row?

    //get vertical
    var theCol = inp[0] % 9;
    //get 3sq mini col
    var theMini = Math.floor(inp[0] / 27);

    //for the 2nd (and possible 3rd) in array
    for (var i = 1; i<inp.length; i++){
        //is this next item in the same group of 3?
        if((Math.floor(inp[i]/27) !== theMini) || (inp[i] % 9 == theCol)){
            return false
        }
    }
...
PossMatrix.prototype.candidateLine = function(){
    /*single items handled elsewhere & max of 3 items in a row/column. If we find the only 2 or 3
    possibilities for a number to be in the same row or column, that larger row/column grid
    needs to remove those possibilities from all other squares therein.
     The opposite can happen from line-> square. This is the first
    solving technique that requires communicating with a different grid. */

    //get the most updated information
    this.update();
    //console.log("candidate line called");
    //for all possibilties
    for(var i = 0; i< this.matrix.length; i++){
        //are there only 2 or 3 possibilities? Only way this method works
        if((this.matrix[i].length == 2 || this.matrix[i].length ==3)){
            //console.log("calling tests for ", this.matrix[i]);
            //same mini row or column?
            if((isSameMiniCol(this.matrix[i]) || isSameMiniRow(this.matrix[i]))){
                //console.log("we've found a candidate match for ", i + 1, "at ", this.matrix[i]);
                var removeVal = i+1;
                //console.log("removal val is ", removeVal);
                //get gridboard to search for the other grid containing these squares and remove from all other squares in grid
                this.puzzle.candidateRemove(removeVal, this.matrix[i], this.type);
